...
class FOGASHyperOptimizer:
    ...
    def optimize_fogas_hyperparameters(
        ...
        # Initialize parameters from bounds instead of theory values
        # For each parameter, resolve its bounds first
        initial_from_bounds = {}
        for param_name in ["alpha", "rho", "eta"]:
            # Get config (default + any overrides)
            cfg = dict(self.DEFAULT_BOUNDS[param_name])
            override = bounds_overrides.get(param_name, None)
            ...
            bspec = cfg["bounds"]
            bounds = bspec(theory, {}) if callable(bspec) else bspec
            
            # Set initial value: lower bound for alpha/eta, upper bound for rho
            if param_name in ["alpha", "eta"]:
                initial_from_bounds[param_name] = float(bounds[0])  # minimum
            else:  # rho
                initial_from_bounds[param_name] = float(bounds[1])  # maximum
        
        # current best-known params start at bound-based values
        current = dict(initial_from_bounds)
        ...
        if print_main:
            ...
            print(f"[Initial] metric = {metrics['theory']:.4f}")
            print(f"Order: {order}")
