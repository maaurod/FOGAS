...
# The ranges are intentionally moderate for the 5x5 tabular problem: wide
...
def next_state(s: int, a: int) -> int:
    ...
    probabilities = torch.tensor([p for _, p in probs], dtype=torch.float)
...
def build_model_arrays() -> tuple[np.ndarray, np.ndarray]:
    ...
    for s in range(N):
        for a in range(A):
            ...
            for sp, prob in transition_probs(s, a):
                P[s, a, sp] += prob
                R[s, a] += prob * reward_fn(s, a, sp)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Grid search SBEED hyperparameters on the stochastic 5x5 wall/pit/goal gridworld."
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "sbeed_5grid_stochastic_grid_search.csv",
...
def main() -> None:
    args = parse_args()
    args.P, args.R = build_model_arrays()
